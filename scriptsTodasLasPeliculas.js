...
const genero = async () =>{
    //Intenta ejecutarme ese codigo
      try{
          const respuestaDTodasP = await (await fetch(`https://api.themoviedb.org/3/movie/popular?api_key=5dfa558924d0c803dbef7f8540472232&language=es-ES`)).json();
          const resultado= respuestaDTodasP.results;
          const generoEspecifico = resultado.filter((pelicula) => {
                return pelicula.genre_ids.includes(28)
            })
            localStorage.setItem("Pelicula",JSON.stringify(generoEspecifico));
            //  console.log(generoEspecifico)
        }catch(error){// Sihay algun error CAtch va a intentar capturarme ese error
            }
        }
    genero()
        let PeliculasAccion =JSON.parse(localStorage.getItem("Pelicula"));
        console.log(PeliculasAccion)
        let selecionListaPelicula =document.querySelector(".acc");
        let listaAccion= "";
        for (let i = 0; i < PeliculasAccion.length; i++) {
            listaAccion += `<option value="">${PeliculasAccion[i].title}</option>`;
            console.log(PeliculasAccion[i].title)
        }
        selecionListaPelicula.innerHTML=listaAccion